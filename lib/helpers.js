...
var ChartPropertyHelper = function () {
  ...
  _createClass(ChartPropertyHelper, [{
    ...
    value: function setZoom(width, height, scale, projection) {
      ...
      var zoom = d3.behavior.zoom().translate([width / 2, height / 2]).scale(scale).scaleExtent([scale / 4, 8 * scale]).on('zoom', zoomed);
    ...
    key: 'setColorLegend',
    value: function setColorLegend(width, height, colors, minValue, maxValue, labelY, legendY, size) {
      // append legend
      var svg = this.chart.select('svg');
      svg.selectAll('.color-legend').remove();
      var legend = svg.append('g').attr('class', 'color-legend');
      ...
      for (var c = 0; c <= maxColor; c++) {
        ...
        legend.append('text').attr('x', labelX).attr('y', labelY).text(value);
