  ...
  _createClass(ChartPropertyHelper, [{
    ...
    value: function setZoom(width, height, scale, projection) {
      ...
      var zoom = d3.behavior.zoom().scaleExtent([1, 8]).on('zoom', function () {
        chart.select('svg').select('g').attr('transform', 'translate(' + d3.event.translate + ') scale(' + d3.event.scale + ')');
      });
    ...
    value: function setColorLegend(width, height, colors, minValue, maxValue, labelY, legendY, size, labelValueFormat, locale) {
      ...
      var format = locale && labelValueFormat ? locale.numberFormat(labelValueFormat ? labelValueFormat : ',.d') : function (value) {
        return value;
      };
